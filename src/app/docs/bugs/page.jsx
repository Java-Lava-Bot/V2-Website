    ...
    bugs: [
      {
        ...
        description: "We have been made aware of issues where the scam bots are starting to become several steps ahead of us with seeing the channel names, and message history, I am going to be adding a system where users can have REQUIRED 3 channel names that will rotate every 72 hours (3 days). I do recommend adding in @Muffin as well because our developer that helps host @Java Lava, @trainer jeo has a major scam prevention update coming out for muffin which will automatically delete the scam stuff within MILLISECONDS of the scam message getting sent, and then once the honeypot system on @Java Lava gets a message sent in that channel, the bot will auto kick that user that is sending scams in that server, and we will be adding a bypass for @Muffin that way muffin doesn't get removed when it sends a message in that channel, all it will do is delete the message by muffin. I will be aiming for this update to come out within the next week or 2, you can add @Muffin in at this link and wait for @trainer jeo to implement the update into production mode, and you can keep JL in your server with @Muffin to help prevent scams, and if it somehow isn't deleted by muffin, then that honeypot channel gets triggered and @Java Lava will delete it.",
        Reproduced: 'We have been made aware of this issue by several users, and we have seen it ourselves as well, we are currently working on a fix for this and it will be updated in the next week or 2.',
      },
      {
